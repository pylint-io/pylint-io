...
class BadgeControllerTest < ActionDispatch::IntegrationTest
  ...
  test "github routing org only svg" do
    ...
    assert_raises(ActionController::RoutingError) do
      get "/github/#{r.repository.owner}.svg"
    end
  end
  ...
  test "github org repo svg" do
    r = ratings(:one)
    get "/github/#{r.repository.owner}/#{r.repository.name}.svg"
    assert_response :success
    assert @response.body.include? r.rating.to_s
  end
  test "github org repo json" do
    r = ratings(:one)
    get "/github/#{r.repository.owner}/#{r.repository.name}", as: :json
    assert_response :success
  ...
  test "github org repo module" do
    r = ratings(:one)
    get "/github/#{r.repository.owner}/#{r.repository.name}/#{r.module}.svg"
    assert_response :success
    assert @response.body.include? r.rating.to_s
  end
  test "github org repo branch module" do
    r = ratings(:two)
    get "/github/#{r.repository.owner}/#{r.repository.name}/#{r.branch}/#{r.module}.svg"
    assert_response :success
    assert @response.body.include? r.rating.to_s
  end
  ...
  test "github org unknown repo" do
    ...
    get "/github/#{r.repository.owner}/#{random}.svg"
    assert_response :success
  ...
  test "github org repo default branch unknown module" do
    r = ratings(:one)
    random = (0...50).map { ('a'..'z').to_a[rand(26)] }.join
    get "/github/#{r.repository.owner}/#{r.repository.name}/#{random}.svg"
    assert_response :success
    assert @response.body.include? "unknown"
  end
  test "github org repo unknown branch" do
    r = ratings(:one)
    random = (0...50).map { ('a'..'z').to_a[rand(26)] }.join
    get "/github/#{r.repository.owner}/#{r.repository.name}/#{random}/#{r.module}.svg"
    assert_response :success
    assert @response.body.include? "unknown"
  end
  test "github org repo branch unknown module" do
    r = ratings(:one)
    random = (0...50).map { ('a'..'z').to_a[rand(26)] }.join
    get "/github/#{r.repository.owner}/#{r.repository.name}/#{r.branch}/#{random}.svg"
    assert_response :success
    assert @response.body.include? "unknown"
  end
end
